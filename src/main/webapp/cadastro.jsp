...
<body class="bg-light d-flex align-items-center justify-content-center" style="min-height: 100vh; padding: 2rem 0;">
    <div class="card border-0 shadow-lg rounded-5 p-5" style="max-width: 500px; width: 100%;">
        <div class="text-center mb-4">
            <h2 class="brand-font" style="color: var(--hero-bg);">Junte-se ao Clube</h2>
            <p class="text-muted">Crie a sua conta e desfrute de benefícios exclusivos.</p>
        </div>

        <% if (request.getAttribute("erro") != null) { %>
            <div class="alert alert-danger small"><%= request.getAttribute("erro") %></div>
        <% } %>

        <form action="cadastro" method="post">
            <div class="mb-3">
                <label class="form-label small fw-bold text-muted">NOME COMPLETO</label>
                <input type="text" name="nome" class="form-control rounded-3 bg-light border-0" required>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="password" name="senha" class="form-control rounded-3 bg-light border-0" required>
            </div>
            <div class="row g-3 mb-4">
                <div class="col-8">
                    <label class="form-label small fw-bold text-muted">ENDEREÇO</label>
                    <input type="text" name="endereco" class="form-control rounded-3 bg-light border-0" placeholder="Rua, Número, Cidade" required>
                </div>
                <div class="col-4">
                    <label class="form-label small fw-bold text-muted">CEP</label>
                    <input type="text" name="cep" class="form-control rounded-3 bg-light border-0" required>
                </div>
            </div>
